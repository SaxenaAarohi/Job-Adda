...
export default function Header() {
    ...
    return (
        <div className="h-12 bg-gray-900 text-black flex justify-between px-4">

            <div>
                <img className="h-[40px] w-20 "
            ...
            <div className="flex relative">
                ...
                <input type="text"
                    value={query}
                    className="w-[500px] border bg-gray-300 h-8 my-2 p-1"
                    onChange={(e) => setQuery(e.target.value)}
                    placeholder="Search Job" />

                <Link
                    href={`/Search?q=${query}`}
                    className="bg-blue-800 h-8 my-2 p-2"
                >
                    Search
                </Link>

                {suggestions.length > 0 && (
                    <ul className="absolute top-10 left-0 w-[500px] bg-gray-200 border rounded-b-md shadow-md z-50">
            ...
            <div className="flex gap-5 text-4xl text-white py-2">
                {
                ...
                <Link href={"/"} title={user?.user?.email} ><CgUser /> </Link>
